...
def valid2(post_data):
    Special_Sym = ['$', '@', '#', '%']
    password = post_data['new password']
...
def valid3(post_data):
    user_name = post_data['update_user_name']
    phone = post_data['update_phone']
    email = post_data["update_email"]
    regex = '^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$'

    if 4 > len(user_name) or len(user_name) > 10:
        return {'Error': 'update_user_name range between 4-10'}
    elif phone == int(phone) and len(str(phone)) != 10:
        return {'Error': 'update_phone number must contain 10 digits'}
    elif 14 > len(email) or len(email) > 20:
        return {'Error': 'update_email range between 13-20'}
